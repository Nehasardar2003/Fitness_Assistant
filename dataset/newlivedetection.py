...
import pickle
import pyttsx3
import time  # Import time for message delay
...
def speak_message(message):
    """Function to announce a message via text-to-speech."""
    engine.say(message)
    engine.runAndWait()
...
def live_pose_count(model_file):
    ...
    message_timer = 0  # Timer for showing the "Keep it up!!!" message
        ...
        if results.pose_landmarks:
            ...
            if prediction == 1:
                if state == "down" and left_angle > 160:  # Arms straight, move to "up"
                    ...
                    count += 1
                    print(f"Count: {count}")
                    speak_message(f"{count}, Keep it up!")  # Speak count + encouragement
                    message_timer = time.time()  # Start the message timer
                    state = "down"
            ...
            # Display "Keep it up!!!" message for 1 second after each count
...
if __name__ == "__main__":
    model_file = "pkl modules/pullup_1.pkl"  # Replace with your actual model file
